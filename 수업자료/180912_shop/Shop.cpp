...
void Shop::SetupItem()
{
	...
	fopen_s(&fp, filePath.c_str(), "r");
	{
		...
		while (!feof(fp))
		{
			...
			item.name =  strtok_s(result, "][", &result);
			item.price = stoi(strtok_s(result, "][", &result));
			item.attribute = stoi(strtok_s(result, "][", &result));
			item.description = strtok_s(result, "][", &result);
			item.count = stoi(strtok_s(result, "][", &result));
