...
void Camera::Update()
{
	...
	D3DXVECTOR3 forward = { 0.0f,0.0f,1.0f }; //카메라의 전방(촬영 방향)
...
void Camera::CreateBuffer()
{
	...
	bufferDesc.ByteWidth = sizeof(VeiwOrthoBufferDesc);
	bufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	HRESULT hr = DEVICE()->CreateBuffer(&bufferDesc, NULL, &ViewOrthoBuffer);
	assert(SUCCEEDED(hr) && "Not Create Veiw and Ortho Buffer");

	//ortho 생성
	D3DXMatrixOrthoOffCenterLH
	(
		&ViewOrthoBufferData.Ortho, 0, 
		desc.Width, 0, desc.Height, 0.0f, 100.0f
	);
