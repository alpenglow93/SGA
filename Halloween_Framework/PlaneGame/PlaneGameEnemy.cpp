...
void BaseEnemy::Move()
{
	...
	D3DXVECTOR2 position;
	plane->Position(&position);
	angle = Intersect::GetAngle(this->position, position) 
		- D3DXToRadian(90.0f);

	//D3DXMATRIX R;
	//D3DXMatrixRotationZ(&R, angle); //회전 행렬
	//direction = { 1.0f, 0.0f }; //기준점 
	//D3DXVec2TransformNormal(&direction, &direction, &R);
	//D3DXVec2Normalize(&direction, &direction);
	
	direction = position - this->position; //상대방의 위치 - 내위치 
...
void BaseEnemy::Death()
{
	...
	{
		position = D3DXVECTOR2( 100 + rand() % 1000, 100 + rand() % 500);
		state = STATE::Idle;
		hp = 100.0f;
